...
from .utils import test_split, train_learner, accuracy, evaluate_learner
...
class MetaLSTM(Trainer):
    ...
    def adapt(self,batch):
        ...
        cI,phi = train_learner(self.learner_w_grad, self.meta_learner, train_input, train_target, self.args)
        ...
        loss = self.loss(output, test_target)        
        acc  = accuracy(output, test_target)
        return loss, acc, phi
    
    def get_init(self):
        return self.meta_learner.metalstm.cI.data

    def set_init(self,flat_params):
        return self.meta_learner.metalstm.init_cI(flat_params)
